...
extension SCNGeometry {
  ...
  static func multiPointsLine(points: [SCNVector3], width:Int = 20, closed: Bool = false, color: CGColor = UIColor.black.cgColor, mitter: Bool = false) -> (SCNNode, SCNGeometry) {
    ...
    lineNode.name = "MultiLineWithWithPoints: \(points.count)"
    
    return (lineNode,geometry);
  }
  ...
  static func createtPlane(baseLinePoints: [SCNVector3], height: Float) -> SCNNode {
    ...
    shapeNode.geometry?.firstMaterial?.transparent.contents = UIColor(red: 1, green: 1, blue: 1, alpha: 0.95)
    ...
    shapeNode.name = "GreenScreenShape: \(baseLinePoints.count)"
    
    return shapeNode
  }
  
  // create geomtry only from baseLinePoints
  static func geometryFromPointsWithNewHeight(baseLinePoints: [SCNVector3], height: Float) -> SCNGeometry {
    ...
    let inds = SCNGeometryElement(indices: indices, primitiveType: .triangleStrip)
    let newGeometry = SCNGeometry(sources: [src], elements: [inds])
    return newGeometry
  }
  ...
  static func cornerNodeAtPosition(_ position: SCNVector3) -> SCNNode {
    ...
    //let cornerNode = SCNSphere(radius: 0.01)
    let cornerNode = SCNCylinder(radius: 0.01, height: 0.001)
    
    cornerNode.firstMaterial?.diffuse.contents = UIColor.white
    
    let node = SCNNode(geometry: cornerNode)
