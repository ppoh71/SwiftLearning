...
class NoteDetailsViewController: UIViewController {
    ...
    @IBAction func cowTapped(sender: Any){
        ...
        backgroundContext?.perform {
            ...
            let cowText = Pathifier.makeMutableAttributedString(for: selectedText, withFont: UIFont(name: "OpenSans-Bold", size: 56)!, withPatternImage: UIImage(named: "texture-cow")!)
...
//        textView.selectedRange = NSMakeRange(selectedRange.location, 1)
    ...
    @IBAction func redTapped(sender: Any){
        ...
        let atttributes: [NSAttributedString.Key: Any] = [
            .foregroundColor: UIColor.red,
            .underlineStyle: 1,
            .underlineColor: UIColor.red]
        
        newText.addAttributes(atttributes, range: textView.selectedRange)
...
extension NoteDetailsViewController {
    func addSaveNotificationObserver(){
         removeSaveNotificationObserver()
         saveObersverToken = NotificationCenter.default.addObserver(forName: .NSManagedObjectContextObjectsDidChange, object: dataController.viewContext, queue: nil, using: handleSaveNotification(notification:))
    }
    
    func removeSaveNotificationObserver(){
        if let token = saveObersverToken{
