...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func configureDatabase() {
        ...
        _refHandle = ref.child("messages").observe(.childAdded, with: { (DataSnapshot) in
            print("added message")
            self.messages.append(DataSnapshot)
            let index = self.messages.count > 0 ? self.messages.count - 1 : 0
            self.messagesTable.insertRows(at: [IndexPath(row: index, section: 0)], with: UITableViewRowAnimation.automatic)
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let text = message[Constants.MessageFields.text] ?? "usertext"
